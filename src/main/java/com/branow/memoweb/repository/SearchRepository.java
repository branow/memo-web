...
public interface SearchRepository extends JpaRepository<User, Integer> {
    ...
    default Page<ModuleSearchingRepositoryDto> searchModuleAllByQuery(String query, Integer pageNumber, Integer pageSize) {
        List<ModuleSearchingRepositoryDto> list = searchModuleAllByQueryInPage(query, pageNumber, pageSize);
        int total = countModuleAllByQuery(query);
        return new PageImpl<>(list, PageRequest.of(pageNumber, pageSize), total);
    }

    @Query(value = "call search_all_module_by_query_in_page(:p_query, :p_page_number, :p_page_size)",nativeQuery = true)
    List<ModuleSearchingRepositoryDto> searchModuleAllByQueryInPage(@Param("p_query") String query,
                                                                @Param("p_page_number") Integer pageNumber,
                                                                @Param("p_page_size") Integer pageSize);

    @Query(value = "call count_all_module_by_query(:p_query)",nativeQuery = true)
    Integer countModuleAllByQuery(@Param("p_query") String query);
